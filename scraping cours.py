...
for i in range(1, 11):  # ici on choisit le nombre de pages qu'on veut charger
    ...
    for i, j, k in zip(soup.find_all('span', class_="text"),  soup.find_all('small', class_="author"), soup.find_all('div', class_='tags')):
        ...
        cur.execute("INSERT INTO citations (Citation, auteur, tags) VALUES(?,?,?)",
                    (cit, auth, ", ".join(tag)))
        conn.commit()
...
with open('citation.csv', 'w', newline="") as csvfile:
    ...
    recueil.append([citation(citatio, auteur, t)
                    for citatio, auteur, t in zip(citations, auteurs, tags)])


# affiche toutes les citations de la base de données venant d'einstein
cur.execute("select * from citations where auteur= 'Albert Einstein'")
for i in cur:
    print(i)
